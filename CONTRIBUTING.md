...
The Brigade Bitbucket Gateway is an official extension of the Brigade project
...
1. Create a service account for the gateway:
   ...
   $ brig service-account create \
       --id bitbucket-gateway \
       --description bitbucket-gateway
   ```
...
1. Grant the service account permission to create events:
   ...
   $ brig role grant EVENT_CREATOR \
     --service-account bitbucket-gateway \
...
## Receiving Events from Bitbucket

Making the gateway that runs in your local, development-grade Kubernetes cluster
visible to Bitbucket such that it can successfully deliver webhooks can be
...
1. Using the URL `<ngrok URL>/events`, complete the usual usage instructions
   ...
   [this section](./README.md#creating-webhooks).
